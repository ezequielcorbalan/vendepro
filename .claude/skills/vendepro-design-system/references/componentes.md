...
**`Modal`** — diálogo centrado. `padded={false}` para contenido a sangre (una banda
de filtros con borde, una lista que scrollea sola).
...
`<p>` con clases sueltas. Los títulos de sección (Heading 2) van en semibold.
