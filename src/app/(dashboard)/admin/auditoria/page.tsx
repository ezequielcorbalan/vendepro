...
import { FileText, ArrowRight, User, Building2, Filter, ChevronLeft, ChevronRight } from 'lucide-react'
import { LEAD_STAGES, PROPERTY_STAGES, type LeadStage } from '@/lib/crm-config'
...
export default function AuditoriaPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/audit?${params}`).then(r => r.json() as Promise<any>).then(d => { setData(d); setLoading(false) }).catch(() => setLoading(false))
  }, [filterType, page])
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
        <div>
          <h1 className="text-xl sm:text-2xl font-semibold text-gray-800 flex items-center gap-2">
            <FileText className="w-6 h-6 text-[#ff007c]" /> Auditor&iacute;a
          </h1>
      ...
      {loading ? (
        <div className="flex items-center justify-center h-40"><div className="w-8 h-8 border-4 border-[#ff007c] border-t-transparent rounded-full animate-spin" /></div>
      ) : results.length === 0 ? (
        <div className="bg-white rounded-xl p-12 text-center">
          <FileText className="w-14 h-14 text-gray-300 mx-auto mb-3" />
          <p className="text-gray-500">No hay registros de auditor&iacute;a</p>
        </div>
      ) : (
        <>
          <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
            <div className="divide-y divide-gray-50">
              {results.map((r: any) => {
                ...
                const Icon = cfg.icon
                const time = new Date(r.created_at).toLocaleString('es-AR', { day: '2-digit', month: '2-digit', year: '2-digit', hour: '2-digit', minute: '2-digit' })
                const entityLink = r.entity_type === 'lead' ? `/leads/${r.entity_id}` : r.entity_type === 'property' ? `/propiedades/${r.entity_id}` : '#'
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-between mt-4">
              <p className="text-xs text-gray-400">P&aacute;gina {page + 1} de {totalPages}</p>
