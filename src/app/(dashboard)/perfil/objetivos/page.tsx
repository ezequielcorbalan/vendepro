...
export default function MisObjetivosPage() {
  ...
  function getPeriodDates(type: string) {
    const now = new Date(Date.now() - 3 * 3600000) // AR
    ...
    if (type === 'quarterly') {
      ...
      return { start: `${y}-${String(qm+1).padStart(2,'0')}-01`, end: `${y}-${String(qm+3).padStart(2,'0')}-${new Date(y,qm+3,0).getDate()}` }
    }
    ...
    <div className="max-w-2xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">{objectives.length} objetivo{objectives.length !== 1 ? 's' : ''} activo{objectives.length !== 1 ? 's' : ''}</p>
        </div>
        <button onClick={() => setShowForm(true)} className="inline-flex items-center gap-1.5 bg-[#ff007c] text-white px-4 py-2 rounded-lg text-sm font-medium hover:opacity-90">
      ...
      {showForm && (
        <div className="bg-white rounded-xl border border-gray-100 p-5 mb-6 space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div>
              <label className="block text-xs font-medium text-gray-600 mb-1">M&eacute;trica</label>
            ...
            <div>
              ...
              <input type="number" value={target} onChange={e => setTarget(e.target.value)}
                className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm" placeholder="10" min="1" />
            </div>
          </div>
              ...
              <div key={obj.id} className="p-4 flex items-center justify-between">
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-0.5">
                    ...
                    <span className="text-xs text-gray-400">{obj.period_start} &rarr; {obj.period_end}</span>
                  </div>
                </div>
