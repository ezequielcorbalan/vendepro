...
export default function EditarTasacionPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        if (data.error) { router.push('/tasaciones'); return }
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const data = (await res.json()) as any
      if (data.success) router.push(`/tasaciones/${id}`)
    ...
    <div className="max-w-3xl mx-auto">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-brand-gray text-sm">{address} — Paso {step + 1} de {steps.length}</p>
        </div>
      </div>
          ...
          return (
            <button key={i} onClick={() => setStep(i)}
              className={`flex items-center gap-1.5 px-2.5 sm:px-4 py-2 rounded-lg text-xs sm:text-sm font-medium whitespace-nowrap transition-colors ${
                i === step ? 'bg-brand-pink text-white' : 'bg-gray-100 text-gray-500 hover:bg-gray-200 cursor-pointer'
              }`}>
      ...
      <div className="bg-white rounded-xl shadow-sm p-4 sm:p-6">
        {step === 0 && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="sm:col-span-2"><label className={labelClass}>Dirección *</label><input className={inputClass} value={address} onChange={e => setAddress(e.target.value)} /></div>
              <div><label className={labelClass}>Barrio *</label><input className={inputClass} value={neighborhood} onChange={e => setNeighborhood(e.target.value)} /></div>
              <div><label className={labelClass}>Ciudad</label><input className={inputClass} value={city} onChange={e => setCity(e.target.value)} /></div>
              <div><label className={labelClass}>Tipología</label>
              ...
              <div><label className={labelClass}>Antigüedad</label><input className={inputClass} type="number" value={propertyAge} onChange={e => setPropertyAge(e.target.value)} /></div>
            </div>
            <div className="border-t border-gray-100 pt-4">
              <h3 className="text-sm font-semibold text-gray-700 mb-3">Superficies (m²)</h3>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <div><label className={labelClass}>Cubierta</label><input className={inputClass} type="number" value={coveredArea} onChange={e => setCoveredArea(e.target.value)} /></div>
                <div><label className={labelClass}>Total</label><input className={inputClass} type="number" value={totalArea} onChange={e => setTotalArea(e.target.value)} /></div>
                <div><label className={labelClass}>Semi</label><input className={inputClass} type="number" value={semiArea} onChange={e => setSemiArea(e.target.value)} /></div>
                <div><label className={labelClass}>Ponderada</label><div className="bg-brand-pink/5 border border-brand-pink/20 rounded-lg px-3 py-2.5 text-sm font-semibold text-brand-pink">{weightedArea.toFixed(1)} m²</div></div>
              </div>
            </div>
            <div className="border-t border-gray-100 pt-4">
              <h3 className="text-sm font-semibold text-gray-700 mb-3">Valores promedio de la zona</h3>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                <div><label className={labelClass}>Precio prom. USD</label><input className={inputClass} type="number" value={zoneAvgPrice} onChange={e => setZoneAvgPrice(e.target.value)} /></div>
                <div><label className={labelClass}>m² promedio</label><input className={inputClass} type="number" value={zoneAvgM2} onChange={e => setZoneAvgM2(e.target.value)} /></div>
                <div><label className={labelClass}>USD/m² prom.</label><input className={inputClass} type="number" value={zoneAvgUsdM2} onChange={e => setZoneAvgUsdM2(e.target.value)} /></div>
              </div>
            </div>
          </div>
        )}
        ...
        {step === 1 && (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div><label className={labelClass}>Fortalezas</label><textarea className={`${inputClass} h-24`} value={strengths} onChange={e => setStrengths(e.target.value)} /></div>
              <div><label className={labelClass}>Debilidades</label><textarea className={`${inputClass} h-24`} value={weaknesses} onChange={e => setWeaknesses(e.target.value)} /></div>
              <div><label className={labelClass}>Oportunidades</label><textarea className={`${inputClass} h-24`} value={opportunities} onChange={e => setOpportunities(e.target.value)} /></div>
              <div><label className={labelClass}>Amenazas</label><textarea className={`${inputClass} h-24`} value={threats} onChange={e => setThreats(e.target.value)} /></div>
            </div>
            <div><label className={labelClass}>Análisis de publicación actual</label><textarea className={`${inputClass} h-28`} value={pubAnalysis} onChange={e => setPubAnalysis(e.target.value)} /></div>
          </div>
        )}
        ...
        {step === 2 && (
          <div className="space-y-4">
            <div className="flex items-center justify-between mb-4">
              ...
              {comparables.length < 6 && <button onClick={addComparable} className="flex items-center gap-1 text-xs text-brand-pink font-medium hover:underline"><Plus className="w-3 h-3" /> Agregar</button>}
            </div>
            {comparables.map((comp, i) => (
              <div key={i} className="border border-gray-100 rounded-xl p-3 sm:p-4 space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  {comparables.length > 1 && <button onClick={() => removeComparable(i)} className="text-red-400 hover:text-red-600"><Trash2 className="w-4 h-4" /></button>}
                </div>
                <input className={inputClass} value={comp.zonaprop_url} onChange={e => updateComparable(i, 'zonaprop_url', e.target.value)} placeholder="Link de ZonaProp (referencia)" />
                <div onPaste={e => handlePaste(i, e)} tabIndex={0}
                  className={`border-2 border-dashed rounded-xl p-4 text-center cursor-pointer focus:outline-none focus:border-brand-pink ${comp.loading ? 'border-indigo-300 bg-indigo-50' : 'border-gray-200 hover:border-brand-pink/50'}`}>
                  {comp.loading ? (
                    <div className="flex items-center justify-center gap-2 text-indigo-600"><Loader2 className="w-5 h-5 animate-spin" /><span className="text-sm">Extrayendo datos...</span></div>
                  ) : (
                    <div>
                      <div className="flex items-center justify-center gap-2 text-gray-400 mb-1"><Clipboard className="w-5 h-5" /><span className="text-sm">Click acá y pegá screenshot (Ctrl+V)</span></div>
                      <label className="inline-flex items-center gap-1 mt-2 text-xs text-indigo-600 cursor-pointer hover:underline">
                        <Image className="w-3 h-3" /> O subí una imagen
                        <input type="file" accept="image/*" className="hidden" onChange={e => { const f = e.target.files?.[0]; if (f) extractFromImage(i, f) }} />
                      </label>
                    </div>
                  )}
                </div>
                {pastedImages[i]?.length > 0 && (
                  <div className="flex gap-2 overflow-x-auto pb-1">
                    {pastedImages[i].map((src, j) => <img key={j} src={src} alt="" className="h-16 rounded-lg border flex-shrink-0" />)}
                  </div>
                )}
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                  <div><label className="text-[10px] text-gray-500">Dirección</label><input className={`${inputClass} text-xs`} value={comp.address} onChange={e => updateComparable(i, 'address', e.target.value)} /></div>
                  <div><label className="text-[10px] text-gray-500">Precio USD</label><input className={`${inputClass} text-xs`} type="number" value={comp.price} onChange={e => updateComparable(i, 'price', e.target.value)} /></div>
                  <div><label className="text-[10px] text-gray-500">m² total</label><input className={`${inputClass} text-xs`} type="number" value={comp.total_area} onChange={e => updateComparable(i, 'total_area', e.target.value)} /></div>
                  <div><label className="text-[10px] text-gray-500">m² cubierto</label><input className={`${inputClass} text-xs`} type="number" value={comp.covered_area} onChange={e => updateComparable(i, 'covered_area', e.target.value)} /></div>
                  <div><label className="text-[10px] text-gray-500">USD/m²</label><input className={`${inputClass} text-xs`} type="number" value={comp.usd_per_m2} onChange={e => updateComparable(i, 'usd_per_m2', e.target.value)} /></div>
                  <div><label className="text-[10px] text-gray-500">Días en venta</label><input className={`${inputClass} text-xs`} type="number" value={comp.days_on_market} onChange={e => updateComparable(i, 'days_on_market', e.target.value)} /></div>
                  <div><label className="text-[10px] text-gray-500">Vistas 30d</label><input className={`${inputClass} text-xs`} type="number" value={comp.views_per_day} onChange={e => updateComparable(i, 'views_per_day', e.target.value)} /></div>
                  <div><label className="text-[10px] text-gray-500">Antigüedad</label><input className={`${inputClass} text-xs`} type="number" value={comp.age} onChange={e => updateComparable(i, 'age', e.target.value)} /></div>
                </div>
              </div>
            ))}
          </div>
        ...
        {step === 3 && (
          <div className="space-y-4">
            ...
            <div className="bg-gray-50 rounded-xl p-4 grid grid-cols-2 gap-4 text-center">
              <div><p className="text-xs text-gray-500">Sup. ponderada</p><p className="text-xl font-bold text-gray-800">{weightedArea.toFixed(1)} m²</p></div>
              <div><p className="text-xs text-gray-500">USD/m² prom.</p><p className="text-xl font-bold text-brand-pink">USD {avgUsdM2.toLocaleString('es-AR')}</p></div>
            </div>
            {avgUsdM2 > 0 && weightedArea > 0 && (
              <div className="bg-brand-pink/5 border border-brand-pink/20 rounded-xl p-4 text-center">
                ...
                <p className="text-2xl font-bold text-brand-pink">USD {Math.round(weightedArea * avgUsdM2).toLocaleString('es-AR')}</p>
              </div>
            )}
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div><label className={labelClass}>Valor sugerido (USD)</label><input className={inputClass} type="number" value={suggestedPrice} onChange={e => setSuggestedPrice(e.target.value)} /></div>
              <div><label className={labelClass}>Prueba 30 días (USD)</label><input className={inputClass} type="number" value={testPrice} onChange={e => setTestPrice(e.target.value)} /></div>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="bg-gray-50 rounded-xl p-4">
                <h3 className="text-sm font-semibold text-gray-700 mb-2"><MapPin className="w-4 h-4 text-brand-pink inline mr-1" />Propiedad</h3>
              ...
              <div className="bg-gray-50 rounded-xl p-4">
                <h3 className="text-sm font-semibold text-gray-700 mb-2"><DollarSign className="w-4 h-4 text-brand-pink inline mr-1" />Tasación</h3>
        ...
        <div className="flex items-center justify-between mt-6 pt-4 border-t border-gray-100">
          {step > 0 ? (
            <button onClick={() => setStep(step - 1)} className="flex items-center gap-2 text-sm text-gray-500 hover:text-gray-700"><ArrowLeft className="w-4 h-4" /> Anterior</button>
          ) : <div />}
          {step < steps.length - 1 ? (
            <button onClick={() => setStep(step + 1)} className="flex items-center gap-2 bg-brand-pink text-white px-5 py-2.5 rounded-lg text-sm font-medium hover:opacity-90">Siguiente <ArrowRight className="w-4 h-4" /></button>
          ) : (
            <button onClick={handleSubmit} disabled={saving || !address}
              className="flex items-center gap-2 bg-brand-pink text-white px-5 py-2.5 rounded-lg text-sm font-medium hover:opacity-90 disabled:opacity-50">
              {saving ? <Loader2 className="w-4 h-4 animate-spin" /> : <CheckCircle className="w-4 h-4" />} Guardar cambios
            </button>
          )}
        </div>
      </div>
