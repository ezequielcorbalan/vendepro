...
const listReportsWithMetrics = vi.fn().mockResolvedValue({
  total: 2,
  results: [
    {
      id: 'r1',
...
vi.mock('@vendepro/infrastructure', async (importOriginal) => {
  ...
  return {
    ...
    D1AnalyticsReportRepository: vi.fn().mockImplementation(() => ({
...
describe('GET /reports', () => {
  beforeEach(() => { listReportsWithMetrics.mockClear() })
  ...
  it('returns raw metric fields per row', async () => {
    const { default: app } = await import('../src/index')
    const res = await app.request('/reports', { method: 'GET' }, { DB: {}, JWT_SECRET: 'secret' })
    const body = await res.json() as any

    expect(body.results[0].impressions).toBe(500)
    expect(body.results[0].portal_visits).toBe(40)
    expect(body.results[0].in_person_visits).toBe(2)
    expect(body.results[0].status).toBe('published')
    expect(body.results[1].status).toBe('draft')
  })
})
