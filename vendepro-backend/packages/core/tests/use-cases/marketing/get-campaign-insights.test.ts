...
function makeRepo(integration: any) {
  return { findByAgent: vi.fn().mockResolvedValue(integration), save: vi.fn() } as any
}
...
describe('GetCampaignInsightsUseCase', () => {
  ...
  it('not_configured si no hay integración guardada', async () => {
    ...
    const out = await uc.execute({ agentId: 'agent_1', since: '2026-07-01', until: '2026-07-06' })
  ...
  it('not_configured si la integración está deshabilitada o sin token', async () => {
    ...
    expect((await uc1.execute({ agentId: 'agent_1', since: 'a', until: 'b' })).status).toBe('not_configured')

    const uc2 = new GetCampaignInsightsUseCase(
      makeRepo({ ...configured, access_token_encrypted: null }), makeInsights(null), decryptOk)
    expect((await uc2.execute({ agentId: 'agent_1', since: 'a', until: 'b' })).status).toBe('not_configured')
  })
  ...
  it('missing_ad_account si falta el ad_account_id', async () => {
    ...
    const out = await uc.execute({ agentId: 'agent_1', since: 'a', until: 'b' })
