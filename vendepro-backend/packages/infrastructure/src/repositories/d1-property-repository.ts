...
export class D1PropertyRepository implements PropertyRepository {
  ...
  async findByOrg(orgId: string, filters?: PropertyFilters): Promise<Property[]> {
    ...
    if (filters?.commercial_stage) { query += ' AND p.commercial_stage = ?'; binds.push(filters.commercial_stage) }
    if (filters?.operation_type) { query += ' AND COALESCE(p.operation_type,\'venta\') = ?'; binds.push(filters.operation_type) }
    if (filters?.search) {
  ...
  async save(property: Property): Promise<void> {
    ...
    await this.db.prepare(`
      INSERT INTO properties (id, org_id, address, neighborhood, city, property_type, rooms, size_m2,
        ...
        agent_id, status, commercial_stage, operation_type, lead_id, created_at, updated_at)
      VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
    ...
    `).bind(
      ...
      o.public_slug, o.cover_photo, o.agent_id, o.status, o.commercial_stage, o.operation_type ?? 'venta', o.lead_id ?? null,
