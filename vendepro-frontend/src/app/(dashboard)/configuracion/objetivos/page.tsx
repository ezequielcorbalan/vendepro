...
import { Target, Plus, Trash2, Save, Users, ChevronDown, ChevronUp, Loader2 } from 'lucide-react'
import { useToast } from '@/components/ui/Toast'
import { OBJECTIVE_METRICS, type ObjectiveMetric } from '@/lib/crm-config'
  ...
  async function saveBatch() {
    ...
    try {
      ...
      if (data.created) {
        ...
        setShowBatch(false); setBatchTargets({}); setBatchAgents([])
      } else {
      ...
      {showBatch && (
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4 sm:p-6 space-y-4">
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => setShowBatch(false)} className="text-gray-500 text-sm hover:text-gray-700 px-3">
              Cancelar
            </button>
          </div>
        </div>
