  ...
  return (
    ...
    >
      ...
      >
        {label}
        {active
          ? (sort.dir === 'asc' ? <ChevronUp className="w-3 h-3 text-brand-pink" /> : <ChevronDown className="w-3 h-3 text-brand-pink" />)
              ...
              {sortedProperties.length === 0 ? (
                ...
                return (
                  <tr key={p.id} className="border-b last:border-0 hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-2.5 text-brand-pink font-medium whitespace-nowrap">
    ...
    <div className="w-64 shrink-0 flex flex-col">
      ...
      <div
        ...
        className={`flex-1 space-y-2 min-h-[180px] max-h-[calc(100dvh-15rem)] overflow-y-auto rounded-xl border p-2 transition-colors ${
          isOver ? 'bg-pink-50 border-brand-pink ring-2 ring-brand-pink/20' : 'bg-gray-50 border-gray-200'
        }`}
      >
...
function CardBody({ p, dragging }: { p: any; dragging?: boolean }) {
  return (
    <div className={`bg-white border rounded-lg p-3 select-none cursor-grab active:cursor-grabbing ${dragging ? 'shadow-lg ring-2 ring-brand-pink/40' : 'border-gray-200'}`}>
      <div className="flex items-start gap-1.5">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="text-xs text-gray-500 mt-1 space-y-0.5">
            ...
            {p.asking_price && (
              <p className="flex items-center gap-1 text-brand-pink font-medium">
