    ...
    <footer className="bg-white border-t border-gray-200 py-10 px-6 text-center text-sm text-gray-600">
      <div className="max-w-4xl mx-auto space-y-3">
        ...
        <div className="flex flex-wrap gap-4 justify-center text-gray-700">
          ...
          {data.email && <a href={`mailto:${data.email}`} className="inline-flex items-center gap-1.5 hover:text-brand-pink"><Mail className="w-4 h-4" />{data.email}</a>}
          {data.whatsapp && <a href={`https://wa.me/${data.whatsapp.replace(/\D/g, '')}`} target="_blank" rel="noopener" className="inline-flex items-center gap-1.5 hover:text-brand-pink"><MessageCircle className="w-4 h-4" />{data.whatsapp}</a>}
          {data.instagram && <a href={`https://instagram.com/${data.instagram.replace('@', '')}`} target="_blank" rel="noopener" className="inline-flex items-center gap-1.5 hover:text-brand-pink"><Instagram className="w-4 h-4" />{data.instagram}</a>}
        </div>
