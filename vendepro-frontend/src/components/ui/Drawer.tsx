 ...
 * izquierda con side="left", para menús de navegación). Portal + scroll-lock +
 * focus-trap + devolución de foco. Cierra con Esc o click en el fondo
 * (mousedown+mouseup, para no cerrar al soltar afuera tras seleccionar).
 */
interface DrawerProps {
  ...
  side?: 'left' | 'right'
  /** Reemplaza el header por defecto (título + cerrar). */
  header?: ReactNode
  /** Sin padding en el cuerpo, para contenido a sangre (una lista, un nav). */
  padded?: boolean
}

export function Drawer({ open, onClose, title, children, footer, width = 'w-[380px]', side = 'right', header, padded = true }: DrawerProps) {
  ...
  return (
    <Portal>
      <div
        className={cn('fixed inset-0 flex bg-black/35', side === 'left' ? 'justify-start' : 'justify-end')}
