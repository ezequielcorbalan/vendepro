...
Worker que envuelve a **Gemini** (proveedor único) para 6 tareas específicas. Modelo: `gemini-3.5-flash-lite` en todas.
...
| POST | `/extract-entity` | ExtractLeadFromTextUseCase | **Gemini** `3.5-flash-lite` — `{text}` → campos de lead |
| POST | `/extract-image` | ExtractLeadFromImageUseCase | **Gemini** `3.5-flash-lite` (visión) — `{imageBase64, mimeType}` → campos de lead |
| POST | `/landings/:id/edit-block` | EditBlockWithAIUseCase | **Gemini** `3.5-flash-lite` — `{prompt, scope, blockId}` → bloque modificado |

## Casos de uso

### `/extract-metrics` (Anthropic)
Agente sube screenshot de Zonaprop / Argenprop / MercadoLibre. Gemini extrae: impresiones, visitas, consultas, llamadas, posición de ranking, precio promedio. Resultado se guarda en `report_metrics` (ver [[Dominio-Reportes]]).
