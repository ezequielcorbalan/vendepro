...
1. ~~🐛 **NotificationBell nunca muestra nada**~~ — **resuelto 2026-09-10**: llamaba a api-crm cuando el endpoint vive en api-admin, esperaba `{notifications}` (el backend devuelve array plano) y con nombres de campo que no existían (`link`/`urgency` vs `link_url`/`kind`). Ahora apunta a admin, mapea `kind`→urgencia, muestra solo no leídas y el descarte persiste vía `PUT /notifications/:id/read` (endpoint + `MarkNotificationReadUseCase` nuevos). Ojo: el único productor de notificaciones hoy es la acción `notify_agent` de automatizaciones — sin automatizaciones activas la campana sigue legítimamente vacía. Ver [[Dominio-Notificaciones]].
...
| Follow-up por inactividad | 🟢 **corre desde el 10-sep-2026**: `SweepTimeBasedAutomationsUseCase` colgado del cron `*/15` de api-crm evalúa los 3 triggers time-based (`lead.sin_contacto_24h` = leads en nuevo/asignado más viejos que N horas; `lead.sin_respuesta_7d` = leads abiertos sin actividad en N días; `property.publicacion_vencida` = mandato `auth_start_date + auth_duration_days` venciendo en N días). Dispara por el mismo camino que cualquier evento (condiciones + rate limit + dedupe `once` → cada entidad alerta una sola vez), con tope de 50 entidades por automatización por tick y filtro contra `automation_runs` para no reprocesar (los `rate_limited` sí reintentan). Candidatos: `D1TimeBasedCandidateRepository`. Pendiente operativo: deploy de api-crm y **activar las recetas** (`sla_contacto_24h`/`lead_frio_7d`/`publicacion_por_vencer` se seedean con `is_active=0`) desde Configuración → Automatizaciones. |
