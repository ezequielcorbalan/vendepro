...
import { ArrowLeft, Plus, FileBarChart, ExternalLink, Clock, CheckCircle2, Trash2, Pencil, Eye } from 'lucide-react'
                ...
                {reports.map((report: any) => (
                  <div key={report.id} className="flex items-center justify-between p-4 hover:bg-gray-50 transition-colors">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {report.status === 'published' && property.public_slug && (
                        <Link href={`/r/${property.public_slug}`} target="_blank" className="inline-flex items-center gap-1 text-xs text-gray-500 hover:text-brand-pink border border-gray-200 rounded-md px-2 py-1" title="Ver reporte público">
                          <Eye className="w-3.5 h-3.5" /> <span className="hidden sm:inline">Ver</span>
                        </Link>
                      )}
                      <Link href={`/propiedades/${id}/reportes/nuevo?edit=${report.id}`} className="inline-flex items-center gap-1 text-xs text-brand-pink hover:bg-pink-50 border border-pink-200 rounded-md px-2 py-1" title="Editar reporte">
